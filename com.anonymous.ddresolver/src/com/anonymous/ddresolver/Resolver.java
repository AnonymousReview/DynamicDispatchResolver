package com.anonymous.ddresolver;
...
import com.anonymous.open.commons.filters.LoopChildFilter;
import com.anonymous.open.pointsto.preferences.PointsToPreferences;
import com.eanonymous.open.java.commons.analysis.CallSiteAnalysis;
import com.eanonymous.open.java.commons.analysis.CommonQueries;
import com.eanonymous.open.java.commons.analysis.SetDefinitions;
...
public class Resolver {
	...
	public static void resetPreferences(){
		...
		CallGraphPreferences.enableRapidTypeAnalysis(true);
		CallGraphPreferences.enableZeroCFAAnalysis(true);
		CallGraphPreferences.enableReachabilityAnalysis(false);
	}
	...
	public static void collectMetrics(File output) throws IOException, InvalidFilterParameterException{
		...
		Q chaCG = ClassHierarchyAnalysis.getInstance().getCallGraph();
		Q rrtaCG = RapidTypeAnalysis.getInstance().getCallGraph();
		Q zerocfaCG = ZeroControlFlowAnalysis.getInstance().getCallGraph();
	...
	public static void tagShouldBeStaticMethodsRRTA(Q candidateMethods){
		for(Node method : candidateMethods.eval().nodes()){
			...
			boolean shouldBeStatic = true;
			Q rrtaPerControlFlowEdges = Common.universe().edges(RapidTypeAnalysis.PER_CONTROL_FLOW);
